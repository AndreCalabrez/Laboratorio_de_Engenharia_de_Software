...
$query_uf = mysqli_query($link,"SELECT sigla_uf, estado FROM uf;");
...
<div class="container">
    ...
    <!-- Jumbotron -->
    <div class="jumbotron">
        <form action="registrarBairro.php" method="get">
            <label>UF: </label><select name="uf">
                <?php while($prod = mysqli_fetch_array($query_uf)) { ?>
                    <option value="<?php echo $prod['sigla_uf'] ?>"><?php echo $prod['estado'] ?></option>
                <?php } ?>
            </select><br />
            <label>Cidade: </label><select name="cidade">
